...
| `src/lib/environment.ts` | Shared isDesktop()/isWebApp() detection (Phase 3.5) |
...
| `src/lib/db/connection.ts` | Replace Tauri SQL with platform-aware factory; import isDesktop from environment.ts |
...
| `src/lib/hooks/workspace.svelte.ts` | Remove Tauri dialog; add desktop/web branching; import from environment.ts |
| `src/lib/components/app/types.ts` | Add 'libsql' SyncMode |
| `src/routes/+layout.svelte` | Phase 3.5: hide right-click prevention + app zoom in webapp; guard openWorkspaceFolder |
| `src/routes/workspace/+layout.svelte` | Webapp workspace init path; conditional sync scheduler (Phase 3.5) |
| `src/routes/workspace/settings/+page.svelte` | libsql sync settings form; Phase 3.5: desktop/web branching in sync + updater sections |
| `src/lib/components/app/layout/toolbar/app-toolbar.svelte` | Phase 3.5: hide window controls, drag region, resize listener; fix toolbar status |
| `src/lib/components/app/layout/workspace/sync-bottom-bar.svelte` | Connected/Disconnected status; Phase 3.5: hide in webapp |
